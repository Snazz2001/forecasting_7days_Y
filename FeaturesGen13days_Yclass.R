...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results13lf$trainSize[which(results13lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results13lf$trainSize[which(results13lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results13lf$trainSize[which(results13lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df2 <- flt1df
  flt1df2$D13_D23Now <- as.numeric(flt1df2$DCP13/flt1df2$bookedlag14)
  flt1df2$D13_D11 <- flt1df2$DCP13/flt1df2$DCP11
  D11_D13_sd <- apply(flt1df2[,c("DCP11","DCP12","DCP13")],1,sd)
  flt1df2$D13_D11_mom <- flt1df2$D13_D11/D11_D13_sd
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP13+bookedlag14, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_13days_pred <- book_pred
...
write.csv(feat_df, file="feat_13days_Y.csv",row.names = F)