...
LRO$DCP5 <- as.numeric(LRO$DCP5)#update
LRO$SYS_PRED <- LRO$DCP5#update
...
gbmpred75_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){#update
...
gbmmodel75_month <- function(flt1df2, m){#update
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP5","DCP1","bookedlag77","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D5_D23Now","D5_D3","D5_D3_mom","booked_75days_pred","deviation")#update
...
flth_data <- read.csv('feat_75days_Y.csv', header = T, stringsAsFactors = FALSE)#update
colnames(flth_data)
...
for(m in 1:12){
  ...
  for(flt_key in unique(flth_data$FLTKEY)){
    ...
    gbmmodel <- gbmmodel75_month(flth_df, m)#update
    if(class(gbmmodel)!="numeric"){
      results <- gbmpred75_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)#update
...
write.csv(perf_details,file="75daysYClassResult.csv",row.names = FALSE)#update
require(tidyr)
perf_details %>% select(syserr,gbmerr) %>% tidyr::gather(model, value, syserr:gbmerr) %>% ggplot(aes(value,color=model))+geom_density()+scale_x_log10()+ggtitle('rel error distribution for 75 days Y Class')
head(tmp)
