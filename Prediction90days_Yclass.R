...
LRO$DCP4 <- as.numeric(LRO$DCP4)#update
LRO$SYS_PRED <- LRO$DCP4#update
...
gbmpred90_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){#update
...
gbmmodel90_month <- function(flt1df2, m){#update
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP4","DCP1","bookedlag77","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D4_D23Now","D4_D2","D4_D2_mom","booked_90days_pred","deviation")#update
...
flth_data <- read.csv('feat_90days_Y.csv', header = T, stringsAsFactors = FALSE)#update
colnames(flth_data)
...
for(m in 1:12){
  ...
  for(flt_key in unique(flth_data$FLTKEY)){
    ...
    gbmmodel <- gbmmodel90_month(flth_df, m)#update
    if(class(gbmmodel)!="numeric"){
      results <- gbmpred90_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)#update
...
with(perf_details[complete.cases(perf_details),],mean(syserr>gbmerr))#win rate:0.5762
write.csv(perf_details,file="90daysYClassResult.csv",row.names = FALSE)#update
require(tidyr)
perf_details %>% select(syserr,gbmerr) %>% tidyr::gather(model, value, syserr:gbmerr) %>% ggplot(aes(value,color=model))+geom_density()+scale_x_log10()+ggtitle('rel error distribution for 90 days Y Class')
head(tmp)
...
LRO$FLTDATE <- as.Date(LRO$FLTDATE)
LRO$DCP4 <- as.numeric(LRO$DCP4)

countna <- function(x){
  sum(is.na(x))
}

gbmpred90_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){
  test_start_year <- max(flt1df2$year[which(flt1df2$month==m)])
  test_month <- as.Date(paste(test_start_year,m,'1',sep = "-"))
  datatest <- flt1df2 %>% filter(year==test_start_year,month==m)
...
#  actual_pred <- actual_pred %>% left_join(lro) %>% mutate(syserr = abs(DCP4-actual)/actual) %>% select(actual,date,pred,FLT_KEY,DCP4,DCP9,DCP16,syserr)
  if(DoCompare){
    lro <- LRO %>% filter(FLT_key==flt_key) %>% select(FLT_key,FLTDATE,DCP4,DCP9,DCP16)
    actual_pred <- actual_pred %>% left_join(lro, by=c("date"="FLTDATE")) %>% mutate(syserr = abs(DCP4-actual)/actual) %>% select(actual,date,pred,FLT_key,DCP4,DCP9,DCP16,syserr)
    actual_pred <- actual_pred %>% mutate(gbmerr = abs(pred-actual)/actual)
    sys_err <- with(actual_pred,median(syserr,na.rm=TRUE))
    results <- list(relerr,actual_pred,relerrsd,relerrmedian,sys_err)
  }else{
...
gbmmodel90_month <- function(flt1df2, m){
  require(gbm)
  require(lubridate)
  if(!m %in% flt1df2$month){
    return(-1)
  ...
  # feats <- c("DCP8","DCP1","bookedlag28","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
  #               ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D8_D28Now","D8_D6","D8_D6_mom","booked_28days_pred","deviation")
  feats <- c("DCP4","DCP1","bookedlag91","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D4_D23Now","D4_D2","D4_D2_mom","booked_90days_pred","bookedlag364","deviation")
  feats <- intersect(colnames(data),feats)
  fitgbm <- gbm(as.formula(paste("BOOKING~",paste(feats,collapse="+"))),data=data,distribution = 'gaussian',n.trees = 500,interaction.depth=2, shrinkage = 0.05)
...
list(relerr,actual_pred,relerrsd,relerrmedian)
flth_data <- read.csv('feat_90days_Y.csv', header = T, stringsAsFactors = FALSE)
colnames(flth_data)
perf_agg <- NA
perf_details <- NA
...
for(flt_key in unique(flth_data$FLTKEY)){
  ...
  flth_df <- subset(flth_data, FLTKEY==flt_key)
  flth_df$FLTDATE <- as.Date(flth_df$FLTDATE)
  gbmmodel <- gbmmodel90_month(flth_df, m)
  if(class(gbmmodel)!="numeric"){
    results <- gbmpred90_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)
    tmp <- data.frame(FLT_key=flt_key,month=m,relerr=results[[1]],relerrsd=results[[3]],relerrmedian=results[[4]])
    if(index==1){
      perf_agg <- tmp
...
summary.gbm(gbmmodel)
perf_details <- perf_details %>% select(actual,date, pred, FLT_key,SYS_PRED,syserr,gbmerr)
with(perf_details[complete.cases(perf_details),],mean(syserr>gbmerr))#win rate:0.570
write.csv(perf_details,file="90daysYClassResult.csv",row.names = FALSE)#update
require(tidyr)
perf_details %>% select(syserr,gbmerr) %>% tidyr::gather(model, value, syserr:gbmerr) %>% ggplot(aes(value,color=model))+geom_density()+scale_x_log10()+ggtitle('rel error distribution for 90 days Y Class')
head(tmp)
