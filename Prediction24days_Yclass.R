...
LRO$DCP10 <- as.numeric(LRO$DCP10)#update
LRO$SYS_PRED <- LRO$DCP10#update
...
gbmpred24_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){#update
...
gbmmodel24_month <- function(flt1df2, m){#update
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP10","DCP1","bookedlag28","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D10_D23Now","D10_D7","D10_D7_mom","booked_24days_pred","deviation")#update
...
flth_data <- read.csv('feat_24days_Y.csv', header = T, stringsAsFactors = FALSE)#update
colnames(flth_data)
...
for(m in 1:12){
  ...
  for(flt_key in unique(flth_data$FLTKEY)){
    ...
    gbmmodel <- gbmmodel24_month(flth_df, m)#update
    if(class(gbmmodel)!="numeric"){
      results <- gbmpred24_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)#update
...
with(perf_details[complete.cases(perf_details),],mean(syserr>gbmerr))#win rate:0.588
write.csv(perf_details,file="24daysYClassResult.csv",row.names = FALSE)#update
require(tidyr)
perf_details %>% select(syserr,gbmerr) %>% tidyr::gather(model, value, syserr:gbmerr) %>% ggplot(aes(value,color=model))+geom_density()+scale_x_log10()+ggtitle('rel error distribution for 24 days Y Class')
head(tmp)
