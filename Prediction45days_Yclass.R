...
LRO$DCP7 <- as.numeric(LRO$DCP7)#update
LRO$SYS_PRED <- LRO$DCP7#update
...
gbmpred45_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){#update
...
gbmmodel45_month <- function(flt1df2, m){#update
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP7","DCP1","bookedlag49","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D7_D23Now","D7_D5","D7_D5_mom","booked_45days_pred","deviation")#update
...
flth_data <- read.csv('feat_45days_Y.csv', header = T, stringsAsFactors = FALSE)#update
colnames(flth_data)
...
for(m in 1:12){
  ...
  for(flt_key in unique(flth_data$FLTKEY)){
    ...
    gbmmodel <- gbmmodel45_month(flth_df, m)#update
    if(class(gbmmodel)!="numeric"){
      results <- gbmpred45_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)#update
...
with(perf_details[complete.cases(perf_details),],mean(syserr>gbmerr))#win rate:0.5764
write.csv(perf_details,file="45daysYClassResult.csv",row.names = FALSE)#update
require(tidyr)
perf_details %>% select(syserr,gbmerr) %>% tidyr::gather(model, value, syserr:gbmerr) %>% ggplot(aes(value,color=model))+geom_density()+scale_x_log10()+ggtitle('rel error distribution for 45 days Y Class')
head(tmp)
