...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results60lf$trainSize[which(results60lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results60lf$trainSize[which(results60lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results60lf$trainSize[which(results60lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df$bookedlag63 <- as.numeric(dplyr::lag(flt1df$BOOKING,ffreq*9))
  ...
  flt1df2 <- flt1df
  flt1df2$D6_D23Now <- as.numeric(flt1df2$DCP6/flt1df2$bookedlag63)
  flt1df2$D6_D4 <- ifelse(flt1df2$DCP4==0,flt1df2$DCP6,flt1df2$DCP6/flt1df2$DCP4)
  
  D6_D4_sd <- apply(flt1df2[,c("DCP6","DCP4","DCP5")],1,sd)
  # D9_D16_sd <- apply(flt1df2[,c("DCP9","DCP10","DCP11","DCP12","DCP13","DCP14","DCP15","DCP16")],1,sd)
  flt1df2$D6_D4_mom <- ifelse(D6_D4_sd==0,flt1df2$D6_D4,flt1df2$D6_D4/D6_D4_sd) 
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP6+bookedlag63, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_60days_pred <- book_pred
...
write.csv(feat_df, file="feat_60days_Y.csv",row.names = F)