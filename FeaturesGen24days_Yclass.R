...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results24lf$trainSize[which(results24lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results24lf$trainSize[which(results24lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results24lf$trainSize[which(results24lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df2 <- flt1df
  flt1df2$D10_D23Now <- as.numeric(flt1df2$DCP10/flt1df2$bookedlag28)
  flt1df2$D10_D7 <- ifelse(flt1df2$DCP7==0,flt1df2$DCP10,flt1df2$DCP10/flt1df2$DCP7)
  
  #D8_D1_sd <- apply(flt1df2[,c("DCP1","DCP2","DCP3","DCP4","DCP5","DCP6","DCP7","DCP8")],1,sd)
  D7_D10_sd <- apply(flt1df2[,c("DCP10","DCP7","DCP8","DCP9")],1,sd)
  # D9_D16_sd <- apply(flt1df2[,c("DCP9","DCP10","DCP11","DCP12","DCP13","DCP14","DCP15","DCP16")],1,sd)
  flt1df2$D10_D7_mom <- ifelse(D7_D10_sd==0,flt1df2$D10_D7,flt1df2$D10_D7/D7_D10_sd)
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP10+bookedlag28, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_24days_pred <- book_pred
...
write.csv(feat_df, file="feat_24days_Y.csv",row.names = F)