...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results20lf$trainSize[which(results20lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results20lf$trainSize[which(results20lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results20lf$trainSize[which(results20lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP11+bookedlag21, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_20days_pred <- book_pred
...
write.csv(feat_df, file="feat_20days_Y.csv",row.names = F)