...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results3lf$trainSize[which(results3lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results3lf$trainSize[which(results3lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results3lf$trainSize[which(results3lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df2 <- flt1df
  flt1df2$D20_D23Now <- as.numeric(flt1df2$DCP20/flt1df2$bookedlag7)
  flt1df2$D20_D9 <- flt1df2$DCP20/flt1df2$DCP9#Need to confirm it is best
  D9_D20_sd <- apply(flt1df2[,c("DCP9","DCP10","DCP11","DCP12","DCP13","DCP14","DCP15","DCP16","DCP17","DCP18","DCP19","DCP20")],1,sd)
  flt1df2$D20_D9_mom <- flt1df2$D20_D9/D9_D20_sd
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP20+bookedlag7, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_3days_pred <- book_pred
...
write.csv(feat_df, file="feat_3days_Y.csv",row.names = F)