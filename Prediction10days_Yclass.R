...
LRO$DCP14 <- as.numeric(LRO$DCP14)
LRO$SYS_PRED <- LRO$DCP14
...
gbmpred10_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){
...
gbmmodel10_month <- function(flt1df2, m){
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP14","DCP1","bookedlag14","bookedlag21","bookedlag28","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D14_D23Now","D14_D11","D14_D11_mom","booked_10days_pred","deviation")
...
flth_data <- read.csv('feat_10days_Y.csv', header = T, stringsAsFactors = FALSE)
...
for(flt_key in unique(flth_data$FLTKEY)){
  ...
  gbmmodel <- gbmmodel10_month(flth_df, m)
  if(class(gbmmodel)!="numeric"){
    results <- gbmpred10_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)
