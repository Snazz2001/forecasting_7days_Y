...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results16lf$trainSize[which(results16lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results16lf$trainSize[which(results16lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results16lf$trainSize[which(results16lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df2 <- flt1df
  flt1df2$D12_D23Now <- as.numeric(flt1df2$DCP12/flt1df2$bookedlag21)
  flt1df2$D12_D10 <- flt1df2$DCP12/flt1df2$DCP10
  D10_D12_sd <- apply(flt1df2[,c("DCP11","DCP12","DCP10")],1,sd)
  flt1df2$D12_D10_mom <- flt1df2$D12_D10/D10_D12_sd
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP12+bookedlag21, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_16days_pred <- book_pred
...
write.csv(feat_df, file="feat_16days_Y.csv",row.names = F)