# The following code is used to generate the features for leg forecasting for 90days
# Zheng Zhu
# 22/11/2016
####################
library(readr)
...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results90lf$trainSize[which(results90lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<210){
    results90lf$trainSize[which(results90lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results90lf$trainSize[which(results90lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df$bookedlag91 <- as.numeric(dplyr::lag(flt1df$BOOKING,ffreq*13))
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP4+bookedlag91, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
...
write.csv(feat_df, file="feat_90days_Y.csv",row.names = F)