...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results75lf$trainSize[which(results75lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results75lf$trainSize[which(results75lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results75lf$trainSize[which(results75lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df$bookedlag77 <- as.numeric(dplyr::lag(flt1df$BOOKING,ffreq*11))#why not calculate the value based on date information.
  
  #bookedlag based on dates alignment.
  dayslag <- 77
  lags <- data.frame(bookedlag=flt1df$BOOKING,laggeddate=flt1df$FLTDATE+dayslag)
  flt1df <- flt1df %>% left_join(lags,by=c("FLTDATE"="laggeddate"))
  ...
  unique_weekinds <- unique(flt1df2$weekind)
  unique_weekinds <- setdiff(unique_weekinds,weekind_max)
  lm_models <- foreach(i=1:length(unique_weekinds),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP5+bookedlag, data=flt1df2[which(flt1df2$weekind==unique_weekinds[i]),]))
  }  
  ...
  #   list(Model=lm(BOOKING~DCP5+bookedlag, data=flt1df2[which(flt1df2$weekind==i),]))
  # }  
  
  # for(i in weekind_min:(weekind_max-1)){
  #   print(i)
  #   model=lm(BOOKING~DCP5+bookedlag, data=flt1df2[which(flt1df2$weekind==i),])
  # }
  
  #prediction based on lm
  clusterExport(cl,"lm_models")
  
  book_pred <- foreach(i=1:(length(lm_models)-1),.combine = 'c') %dopar%{
    list(list(booked_lm_pred = round(predict(lm_models[[i]],newdata=flt1df2[which(flt1df2$weekind==unique_weekinds[i+1]),]))))
  }
  ...
  #head(book_pred,20)
  flt1df2$booked_75days_pred <- book_pred
...
write.csv(feat_df, file="feat_75days_Y.csv",row.names = F)