...
for(flt_key in unique(flth$FLT_key)){
  ...
  if(i>1&i<nrow(flt1df)){
    ...
    results45lf$trainSize[which(results45lf$FLT_KEY==flt_key)] = 0
    next
  }
  i <- nrow(flt1df)
  ...
  if(nrow(flt1df)<150){
    results45lf$trainSize[which(results45lf$FLT_KEY==flt_key)] = nrow(flt1df)     
    next
  }
  results45lf$trainSize[which(results45lf$FLT_KEY==flt_key)] = nrow(flt1df)  
  ...
  flt1df$bookedlag49 <- as.numeric(dplyr::lag(flt1df$BOOKING,ffreq*7))
  ...
  flt1df2 <- flt1df
  flt1df2$D7_D23Now <- as.numeric(flt1df2$DCP7/flt1df2$bookedlag49)
  flt1df2$D7_D5 <- ifelse(flt1df2$DCP5==0,flt1df2$DCP7,flt1df2$DCP7/flt1df2$DCP5)
  
  D5_D7_sd <- apply(flt1df2[,c("DCP5","DCP6","DCP7")],1,sd)
  # D9_D16_sd <- apply(flt1df2[,c("DCP9","DCP10","DCP11","DCP12","DCP13","DCP14","DCP15","DCP16")],1,sd)
  flt1df2$D7_D5_mom <- ifelse(D5_D7_sd==0,flt1df2$D7_D5,flt1df2$D7_D5/D5_D7_sd)
  ...
  lm_models <- foreach(i=(weekind_min):(weekind_max-1),.combine = 'c') %dopar%{#i is the weekind,start from 2 instead of 1 is due to bookedlag7 is NA for 1.
    list(Model=lm(BOOKING~DCP7+bookedlag49, data=flt1df2[which(flt1df2$weekind==i),]))
  }  
  ...
  #head(book_pred,20)
  flt1df2$booked_45days_pred <- book_pred
...
write.csv(feat_df, file="feat_45days_Y.csv",row.names = F)