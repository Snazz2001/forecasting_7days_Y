...
LRO$DCP20 <- as.numeric(LRO$DCP20)
LRO <- LRO %>% rename(SYS_PRED=DCP20)

countna <- function(x){
  sum(is.na(x))
}

gbmpred3_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){
...
gbmmodel3_month <- function(flt1df2, m){
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP20","DCP1","bookedlag7","bookedlag14","bookedlag21","bookedlag28","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D20_D23Now","D20_D9","D20_D9_mom","booked_3days_pred","deviation")
...
flth_data <- read.csv('feat_3days_Y.csv', header = T, stringsAsFactors = FALSE)
...
for(flt_key in unique(flth_data$FLTKEY)){
  ...
  gbmmodel <- gbmmodel3_month(flth_df, m)
  if(class(gbmmodel)!="numeric"){
    results <- gbmpred3_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)
