...
LRO$DCP8 <- as.numeric(LRO$DCP8)#update
LRO$SYS_PRED <- LRO$DCP8#update
...
gbmpred33_month <- function(flt1df2,fitgbm,m,flt_key,DoCompare=TRUE){#update
...
gbmmodel33_month <- function(flt1df2, m){#update
  require(gbm)
  require(lubridate)
  ...
  feats <- c("DCP8","DCP1","bookedlag35","wday2","wday3","wday4","wday5","wday6","wday7","month2","month3","month4","month5
             ","month6","month7","month8","month9","month10","month11","month12","wdayindex","D8_D23Now","D8_D6","D8_D6_mom","booked_33days_pred","deviation")#update
...
flth_data <- read.csv('feat_33days_Y.csv', header = T, stringsAsFactors = FALSE)#update
colnames(flth_data)
...
for(m in 1:12){
  ...
  for(flt_key in unique(flth_data$FLTKEY)){
    ...
    gbmmodel <- gbmmodel33_month(flth_df, m)#update
    if(class(gbmmodel)!="numeric"){
      results <- gbmpred33_month(flth_df,gbmmodel,m,flt_key,DoCompare=TRUE)#update
...
with(perf_details[complete.cases(perf_details),],mean(syserr>gbmerr))#win rate:0.576
write.csv(perf_details,file="33daysYClassResult.csv",row.names = FALSE)#update
require(tidyr)
perf_details %>% select(syserr,gbmerr) %>% tidyr::gather(model, value, syserr:gbmerr) %>% ggplot(aes(value,color=model))+geom_density()+scale_x_log10()+ggtitle('rel error distribution for 33 days Y Class')
head(tmp)
